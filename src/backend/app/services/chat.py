...
class ChatService:
    ...
    async def process_message(self, message: str, session_id: str) -> RAGResult:
        ...
        logger.info(f"Processing message: '{message[:100]}...' for session: {session_id}")
        
        # 1. Get chat history
        messages = await self.history_manager.get_messages(session_id, limit=settings.chat_history_limit)
