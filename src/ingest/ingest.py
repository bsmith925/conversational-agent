...
def create_embeddings_batch(texts: List[str], model: SentenceTransformer) -> List[List[float]]:
    """
...
async def process_pdf_async(filepath: Path, loop: asyncio.AbstractEventLoop) -> List[Dict[str, Any]]:
    """
...
async def create_embeddings_async(texts: List[str], 
                                 ...
                                 loop: asyncio.AbstractEventLoop) -> List[List[float]]:
    """
    Async wrapper for embedding creation.
    """
    return await loop.run_in_executor(CPU_EXECUTOR, create_embeddings_batch, texts, model)
...
async def insert_chunks_executemany(pool: AsyncConnectionPool, 
...
async def process_pdfs_concurrently(pdf_files: List[Path], 
    ...
    for i in range(0, len(pdf_files), max_concurrent_pdfs):
        ...
        pdf_tasks = [
            process_pdf_async(filepath, loop) 
            for filepath in pdf_batch
        ]
